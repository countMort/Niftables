...
export default function Header() {
  return (
    <header className="fixed z-50 px-5 sm:px-[3.125rem] py-[1.875rem] w-full">
      <nav className="flex justify-between max-sm:flex-col">
        <Link href="/" className="max-sm:order-2">
          <Image
            src={logo_img}
            alt="CREON"
            className="w-[8.75rem] max-sm:mt-1"
          />
        </Link>
        <ul className="flex max-sm:justify-between sm:space-x-8 font-bold my-auto items-center">
          ...
          <li>
            <a
              href="#"
              className="hover:text-gray-200 py-[0.75rem] px-[1.875rem] block border rounded-md"
            >
              Connect
            </a>
          </li>
